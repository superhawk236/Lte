...
LteChannel::StartTx (PacketBurst* p, TransmittedSignal* txSignal, NetworkNode* src)
{
  ...
  //if(src->GetNodeType()==NetworkNode::TYPE_BUS)txdelay=0.015;//mouan

  Simulator::Init()->Schedule(txdelay,
	 						  &LteChannel::StartRx,
	  						  this,
	 						  p,
	  						  txSignal,
...
LteChannel::StartRx (PacketBurst* p, TransmittedSignal* txSignal, NetworkNode* src)
{
  ...
  for (std::vector<NetworkNode*>::iterator it = GetDevices ()->begin();
		  it != GetDevices ()->end (); it++)
    {
	  ...
	  if(dst->GetIDNetworkNode()==src->GetIDNetworkNode())continue;//mouan
	  ...
	  if (m_propagationLossModel != NULL)
	    {
		  ...
		  //mouan
		  //if(txSignal==NULL)
		  //std::cout << "txSignal==NULL" << std::endl;
		  //else
//std::cout << "txSignal!=NULL" << std::endl;
//		  txSignal->Copy();
//std::cout << "txSignaliscopyed" << std::endl;
	  ...
	  //if(p==NULL)std::cout<<"ERROR: p is null"<<std::endl;//test
