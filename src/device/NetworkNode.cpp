	  ...
	  if (packet->GetDestinationMAC () == GetIDNetworkNode ())
	    {
		  if (packet->GetRLCHeader () == NULL)
          ...
          else
            {
			  ...
			  if(bearer->GetRlcEntity()==NULL)return;//???
	    ...
	    {
		  //NOT FOR ME
			/*std::cout<<"Relaying"<<std::endl;//test
			SendPacketBurst(p);*/
			if(GetNodeType()==TYPE_BUS && packet->GetSourceMAC()!=GetIDNetworkNode()){
				//std::cout<<"Relaying"<<std::endl;//test
				p->relayed=GetIDNetworkNode();
				SendPacketBurst(p->Copy());
			}
	    }
    }
}
