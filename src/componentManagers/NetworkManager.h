...
public:
	...
	std::vector<UserEquipment*>*
	GetRegisteredUEToBus (int idBus);
	...
	Cell*
	GetBusBelongingCellFromPosition(Bus* bus);
	void
	SelectBusTargetNode (Bus* bus);

	bool
	CheckHandoverNeed (int idUserEquipment);
	bool
	CheckBusHandoverNeed (int idBus);
	bool
	...
	void
	BusHandoverProcedure (double time, Bus* bus, NetworkNode* oldTarget, NetworkNode* newTarget);
	void
	TransferBearerInfo (UserEquipment* ue, NetworkNode* target);
	void
	BusTransferBearerInfo (Bus* bus, NetworkNode* target);
