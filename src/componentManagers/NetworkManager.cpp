...
NetworkManager::NetworkManager()
{
  ...
  m_BusContainer = new std::vector<Bus*>;
...
NetworkManager::~NetworkManager()
{
  ...
  std::vector<Bus*>::iterator iter5;
  for (iter5 = m_BusContainer->begin();
      iter5 != m_BusContainer->end(); iter5++)
    {
    delete *iter5;
    }
  delete m_BusContainer;
...
Bus*
NetworkManager::CreateBus (int id,
		                      Cell* cell,
		                      double pos_X, double pos_Y,
		                      LteChannel *dlCh, LteChannel *ulCh,
		                      BandwidthManager *bm,ENodeB *enb)
{
  Bus* bus = new Bus (id, cell, enb, pos_X, pos_Y); //Bus (1, cell);
  bus->GetPhy ()->SetDlChannel (enb->GetPhy()->GetDlChannel());//mouan problem : should 3 channels?
  bus->GetPhy ()->SetUlChannel (enb->GetPhy()->GetUlChannel());
  bus->GetPhy ()->SetBandwidthManager (bm);

  FullbandCqiManager *cqiManager = new FullbandCqiManager ();
  cqiManager->SetCqiReportingMode (CqiManager::PERIODIC);
  cqiManager->SetReportingInterval (1);
  cqiManager->SetDevice (bus);
  bus->SetCqiManager (cqiManager);

  enb->RegisterBus (bus);

  //tempory ignore channel realization
  /*channel realization*/

  ulCh->AddDevice (bus);

  GetBusContainer ()->push_back (bus);
...
std::vector<Bus*>*
NetworkManager::GetBusContainer (void)
{
  return m_BusContainer;
}
...
Bus*
NetworkManager::GetBusByID (int idBus)
{
  std::vector<Bus*>* BusContainer = GetBusContainer ();
  std::vector<Bus*>::iterator iter5;
  Bus *Bus;
  for (iter5 = BusContainer->begin ();
      iter5 != BusContainer->end (); iter5++)
    {
    Bus = *iter5;
    if (Bus->GetIDNetworkNode() == idBus)
      {
      return Bus;
      }
    }
...
NetworkManager::GetUserEquipmentByID (int idUE)
{
  ...
  //std::cout<<"NM:GetUEbyID false"<<std::endl;//test
  return false;
}
...
std::vector<UserEquipment*>*
NetworkManager::GetRegisteredUEToBus (int idBus)
{
  std::vector<UserEquipment*>* UElist = new std::vector<UserEquipment*>;
  ...
  for (iter = userEquipmentContainer->begin ();
    ...
    {
    userEquipment = *iter;
    if (userEquipment->GetTargetNode ()->GetIDNetworkNode() == idBus)
      {
      UElist->push_back(userEquipment);
      }
    }
...
NetworkManager::UpdateUserPosition (double time)
{
  ...
  std::cout << "time: " << time << "\t position: "
      ...
      if (record->GetMobilityModel ()->GetHandover () == true)
        {
//std::cout<<"handover is open"<<std::endl;//test
    	  NetworkNode* targetNode = record->GetTargetNode ();

          if (targetNode->GetProtocolStack ()->GetRrcEntity ()->
        		  GetHandoverEntity ()->MouanCheckHandoverNeed (record))
            {
  ...
  //mouan
  std::vector<Bus*> *busrecords = GetBusContainer ();
  std::vector<Bus*>::iterator busiter;
  Bus *busrecord;

#ifdef MOBILITY_DEBUG
  std::cout << "MOBILITY_DEBUG: UPDATE POSITION, "
      "number of Bus = " << busrecords->size () <<
      " time = " << time << std::endl;
#endif
...
#ifdef MOBILITY_DEBUG
    std::cout << "\t BUS  " << busrecord->GetIDNetworkNode ()
	              << std::endl;
#endif

      busrecord->UpdateUserPosition (time);
      //busrecord->SetIndoorFlag( CheckIndoorUsers(busrecord) );

#ifdef AMC_MAPPING
  std::cout << "time: " << time << "\n\t position: "
		  << busrecord->GetMobilityModel ()->GetAbsolutePosition ()->GetCoordinateX () <<
		  " " << busrecord->GetMobilityModel ()->GetAbsolutePosition ()->GetCoordinateY ()
		  << std::endl;
#endif
#ifdef MOBILITY_DEBUG
  std::cout << "time: " << time << "\t position: "
      ...
      if (busrecord->GetMobilityModel ()->GetHandover () == true)
        {
//std::cout<<"handover is open"<<std::endl;//test
    	  NetworkNode* targetNode = busrecord->GetTargetNode ();

          if (targetNode->GetProtocolStack ()->GetRrcEntity ()->
        		  GetHandoverEntity ()->CheckBusHandoverNeed (busrecord))
            {
        	  NetworkNode* newTagertNode = targetNode->GetProtocolStack ()
        			  ->GetRrcEntity ()->GetHandoverEntity ()->GetHoManager ()->m_target;

#ifdef HANDOVER_DEBUG
              std::cout << "** HO ** \t time: " << time << " user " <<  busrecord->GetIDNetworkNode () <<
            		  " old eNB " << targetNode->GetIDNetworkNode () <<
            		  " new eNB " << newTagertNode->GetIDNetworkNode () << std::endl;
#endif
              BusHandoverProcedure(time, busrecord, targetNode, newTagertNode);
            }
        }
    }
}
...
NetworkManager::CheckHandoverPermissions (NetworkNode* target, UserEquipment* user)
{
	...
	else if ( target->GetNodeType() == NetworkNode::TYPE_BUS )
	{
		return true;
	}
...
Cell*
NetworkManager::GetBusBelongingCellFromPosition(Bus* bus)
{
	CartesianCoordinates* bus_pos = bus->GetMobilityModel()->GetAbsolutePosition();
	
		// ue is outdoor and it will be attached to a macrocell
		return GetCellByID( GetCellIDFromPosition( bus_pos->GetCoordinateX(), bus_pos->GetCoordinateY() ) );
...
NetworkManager::SelectTargetNode (UserEquipment* ue)
{
   ...
   if (targetNode->GetProtocolStack ()->GetRrcEntity ()->
		   GetHandoverEntity ()->MouanCheckHandoverNeed (ue))
     {
...
void
NetworkManager::SelectBusTargetNode (Bus* bus)
{
   NetworkNode* targetNode = bus->GetTargetNode ();

   if (targetNode->GetProtocolStack ()->GetRrcEntity ()->
		   GetHandoverEntity ()->CheckBusHandoverNeed (bus))
     {
	   NetworkNode* newTagertNode =
			   targetNode->GetProtocolStack ()->GetRrcEntity ()->
			   GetHandoverEntity ()->GetHoManager ()->m_target;

	   bus->SetTargetNode (newTagertNode);
     }
}
...
{
	if(oldTarget->GetIDNetworkNode()==newTarget->GetIDNetworkNode()){
		std::cout<<"ERROR: HP old = new"<<std::endl;
		return;
	}
#ifdef HANDOVER_DEBUG
  ...
  std::cout << "user " <<  ue->GetIDNetworkNode () << " starts HO "
      "\n\t old eNB = " << ue->GetTargetNode ()->GetIDNetworkNode () << "=" <<oldTarget->GetIDNetworkNode()<<
...
//mouan
void
NetworkManager::BusHandoverProcedure(double time, Bus* bus, NetworkNode* oldTarget, NetworkNode* newTarget)
{
#ifdef HANDOVER_DEBUG
              std::cout << "** HO ** \t BusHandoverProcedure for user "
            		  <<  bus->GetIDNetworkNode () << std::endl;
#endif

  // 1 - deactivate the UE for a time equal to 30 ms
	if (bus->GetNodeState() == Bus::STATE_ACTIVE)
    {
    //XXX Admission Control is not implemented
	double detachTime = 0.030;
    bus->SetNodeState (Bus::STATE_DETACHED);
    Simulator::Init()->Schedule(detachTime, &NetworkNode::MakeActive, bus);
    }

#ifdef MOBILITY_DEBUG
  std::cout << "user " <<  bus->GetIDNetworkNode () << " starts HO "
      "\n\t old eNB = " << bus->GetTargetNode ()->GetIDNetworkNode () <<
      "\n\t target eNB = " <<  newTarget->GetIDNetworkNode () << std::endl;
#endif

  // 2 - transfer all active radio bearer
  Simulator::Init()->Schedule(0.001,
                              &NetworkManager::BusTransferBearerInfo,
                              this,
                              bus,
                              newTarget);

  bus->GetMobilityModel ()->SetLastHandoverTime(time);
}
...
NetworkManager::TransferBearerInfo (UserEquipment* ue, NetworkNode* target)
{
              ...
              std::cout << "** HO ** \t TransferBearerInfo for user "
            		  <<  ue->GetIDNetworkNode () <<" from "<<ue->GetTargetNode()->GetIDNetworkNode()
					  << " to " <<target->GetIDNetworkNode()
					  << std::endl;
#endif
			  if(ue->GetTargetNode()->GetIDNetworkNode()==target->GetIDNetworkNode()){
				  std::cout<<"ERROR: same target HO"<<std::endl;
				  return;
			  }
  if ( (target->GetNodeType() == NetworkNode::TYPE_ENODEB && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_ENODEB) ||
		 ...
		 //(target->GetNodeType() == NetworkNode::TYPE_BUS && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_ENODEB) ||
		 //(target->GetNodeType() == NetworkNode::TYPE_ENODEB && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_BUS) ||
		 //(target->GetNodeType() == NetworkNode::TYPE_BUS && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_BUS))
  {
//	  std::cout<<"Enb to Enb"<<std::endl;//test
	...
	else if ( (target->GetNodeType() == NetworkNode::TYPE_BUS && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_ENODEB))
  {
//	  std::cout<<"Enb to Bus"<<std::endl;//test
	   ENodeB *oldTargetNode = (ENodeB*) ue->GetTargetNode ();
	   Bus *newTargetNode = (Bus*) target;
	...
	else if ( (target->GetNodeType() == NetworkNode::TYPE_ENODEB && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_BUS))
  {
//	  std::cout<<"Bus to Enb"<<std::endl;//test
	   Bus *oldTargetNode = (Bus*) ue->GetTargetNode ();
	   ENodeB *newTargetNode = (ENodeB*) target;
	...
	else if (target->GetNodeType() == NetworkNode::TYPE_BUS && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_BUS)
	{
		return ;
//		std::cout<<"Bus to Bus"<<std::endl;//test
		Bus *oldTargetNode = (Bus*) ue->GetTargetNode ();
	   Bus *newTargetNode = (Bus*) target;
...
//	std::cout<<"End TBI"<<std::endl;//test
}

void
NetworkManager::BusTransferBearerInfo (Bus* bus, NetworkNode* target)
{

#ifdef HANDOVER_DEBUG
              std::cout << "** HO ** \t TransferBearerInfo for bus "
            		  <<  bus->GetIDNetworkNode () << std::endl;
#endif

  if ( (target->GetNodeType() == NetworkNode::TYPE_ENODEB && bus->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_ENODEB) )// ||
		 //(target->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_ENODEB) ||
		 //(target->GetNodeType() == NetworkNode::TYPE_ENODEB && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION) ||
		 //(target->GetNodeType() == NetworkNode::TYPE_BUS && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_ENODEB) ||
		 //(target->GetNodeType() == NetworkNode::TYPE_ENODEB && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_BUS) ||
		 //(target->GetNodeType() == NetworkNode::TYPE_BUS && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_BUS))
  {
	   ENodeB *oldTargetNode = (ENodeB*) bus->GetTargetNode ();
	   ENodeB *newTargetNode = (ENodeB*) target;
	   ...
	   std::cout << "update spectrum, channels and propagation loss model"<< std::endl;
#endif
	   bus->GetPhy ()->SetBandwidthManager (newTargetNode->GetPhy ()->GetBandwidthManager ());
	   ...
	   bus->GetPhy ()->SetDlChannel (newDl);
	   if (oldDl->IsAttached (bus))
	     {
		   newDl->AddDevice (bus);
           oldDl->DelDevice (bus);
	     }
	   if (newDl->GetPropagationLossModel () != NULL)
	     {
		   newDl->GetPropagationLossModel ()->
				   AddChannelRealization (CreateChannelRealization (newTargetNode, bus));

		   oldDl->GetPropagationLossModel ()->DelChannelRealization (oldTargetNode,bus);

	     }

	   bus->GetPhy ()->SetUlChannel (newUl);
	   if (oldUl->IsAttached (bus))
	     {
		   newUl->AddDevice (bus);
           oldUl->DelDevice (bus);
	     }
	   if (newUl->GetPropagationLossModel () != NULL)
	     {
		   newUl->GetPropagationLossModel ()->AddChannelRealization (
				   CreateChannelRealization (bus, newTargetNode));

		   oldUl->GetPropagationLossModel ()->DelChannelRealization (bus,oldTargetNode);;
	     }


	   // 2 - add ue record to the new Enb
#ifdef HANDOVER_DEBUG
	   std::cout << "add bus record to the new Enb"<< std::endl;
#endif
       newTargetNode->RegisterBus (bus);

	   // 3 - delete ue record form the old enb
#ifdef HANDOVER_DEBUG
       std::cout << "delete bus record form the old enb"<< std::endl;
#endif
	   oldTargetNode->DeleteBus (bus);

	   // 4 - update cell and new target enb for the ue
#ifdef HANDOVER_DEBUG
       std::cout << "update cell and new target enb for the bus"<< std::endl;
#endif
	   bus->SetTargetNode (newTargetNode);
	   ...
	   RrcEntity * oldEnbRrc = oldTargetNode->GetProtocolStack ()->GetRrcEntity ();
	   RrcEntity * newEnbRrc = newTargetNode->GetProtocolStack ()->GetRrcEntity ();
	   RrcEntity * busRrc = bus->GetProtocolStack  ()->GetRrcEntity ();
	   ...
	   for (std::vector<RadioBearer* >::iterator it = oldEnbRrc->GetRadioBearerContainer ()->begin();
	     ...
	     {
		   RadioBearer *bearer = (*it);
		   if (bearer->GetDestination ()->GetIDNetworkNode () == bus->GetIDNetworkNode ())
		     {
			   bearer->SetSource (newTargetNode);
			   bearer->GetRlcEntity ()->SetDevice (newTargetNode);
			   newEnbRrc->AddRadioBearer (bearer);
	   ...
	   for (std::vector<RadioBearerSink* >::iterator it = oldEnbRrc->GetRadioBearerSinkContainer ()->begin();
	     ...
	     {
		   RadioBearerSink *bearer = (*it);
		   if (bearer->GetSource ()->GetIDNetworkNode () == bus->GetIDNetworkNode ())
		     {
			   bearer->SetDestination (newTargetNode);
			   newEnbRrc->AddRadioBearerSink (bearer);
			   ulBearerToDelete->push_back (bearer);
	   ...
	   // UPDATE THE RRC CONTEXT FOR THE UE
#ifdef HANDOVER_DEBUG
	   std::cout << "\t bus updates DL radio bearers "<< std::endl;
#endif
	   for (std::vector<RadioBearerSink* >::iterator it = busRrc->GetRadioBearerSinkContainer ()->begin();
			   it != busRrc->GetRadioBearerSinkContainer ()->end (); it++)
	     {
		   RadioBearerSink *bearer = (*it);
		   bearer->SetSource (newTargetNode);
	     }
#ifdef HANDOVER_DEBUG
	   std::cout << "\t bus updates UL radio bearers "<< std::endl;
#endif
	   for (std::vector<RadioBearer* >::iterator it = busRrc->GetRadioBearerContainer ()->begin();
			   it != busRrc->GetRadioBearerContainer ()->end (); it++)
	     {
		   RadioBearer *bearer = (*it);
		   bearer->SetDestination (newTargetNode);
	     }
...
NetworkManager::PrintUEsForEachCell (void)
{
  ...
  std::vector<Bus*> *records2 = GetBusContainer();
  std::vector<Bus*>::iterator iter2;

  std::cout << "UEsForCell(bus): ";
  for (iter2 = records2->begin(); iter2 !=records2->end(); iter2++)
    {
    std::cout << " " << (*iter2)->GetNbOfUserEquipmentRecords();
    }
  std::cout << std::endl;
}
