...
Interference::ComputeInterference (UserEquipment *ue)
{
  ...
  std::vector<Bus*>::iterator bit;

  //std::cout << "Compute interference for UE " << ue->GetIDNetworkNode () << " ,target node " << ue->GetTargetNode ()->GetIDNetworkNode ()<< std::endl;

  for (bit = buses->begin (); bit != buses->end (); bit++)
    {
	  bus = (*bit);
	  if (bus->GetIDNetworkNode () != ue->GetTargetNode ()->GetIDNetworkNode () &&
			  bus->GetPhy ()->GetBandwidthManager ()->GetDlOffsetBw () ==
			  ue->GetTargetNode ()->GetPhy ()->GetBandwidthManager ()->GetDlOffsetBw ())
	    {
		  double powerTXForSubBandwidth = 10 * log10 (
				  pow (10., (bus->GetPhy()->GetTxPower() - 30)/10)
				  /
				  bus->GetPhy()->GetBandwidthManager ()->GetDlSubChannels().size ());


          double nodeInterference_db = powerTXForSubBandwidth - 10 - ComputePathLossForInterference (bus, ue); // in dB
          double nodeInterference = pow(10, nodeInterference_db/10);
