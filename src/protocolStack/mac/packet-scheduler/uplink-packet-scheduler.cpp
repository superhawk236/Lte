...
UplinkPacketScheduler::CreateUsersToSchedule (void)
{
  ...
  //std::cout<<"UTS()"<<std::endl;//memorytest
}
...
UplinkPacketScheduler::SelectUsersToSchedule ()
{
  ...
  if(GetMacEntity ()->GetDevice ()->GetNodeType()==NetworkNode::TYPE_BUS){//5 : TYPE_BUS
	  Bus *node = (Bus*) GetMacEntity ()->GetDevice ();
	  Bus::UserEquipmentRecords *records = node->GetUserEquipmentRecords ();
	  Bus::UserEquipmentRecord *record;
	  Bus::UserEquipmentRecords::iterator iter;
...
UplinkPacketScheduler::DoStopSchedule (void)
{
  ...
  //std::cout<<"PMICM"<<std::endl;//memorytest
  UsersToSchedule* users = GetUsersToSchedule ();
	  ...
	  if (user->m_transmittedData > 0)
		{
		  ...
		  if(GetMacEntity ()->GetDevice ()->GetNodeType()==NetworkNode::TYPE_BUS){//TYPE_BUS = 5
			  Bus *bus = (Bus*) GetMacEntity ()->GetDevice ();
			  Bus::UserEquipmentRecord* record = bus->GetUserEquipmentRecord (user->m_userToSchedule->GetIDNetworkNode ());
